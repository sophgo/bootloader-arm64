...
function se_install_prepackages()
{
	# for se only
	if [ ! -f /root/se_ctrl/debs/installed ] ; then

		# install tftp server and config it
		if [ -d /root/se_ctrl/debs ]; then
			/root/se_ctrl/debs/nfs_setup.sh
			touch /root/se_ctrl/debs/installed
		fi
...
function se_init()
{
	...
	if [ "$product" = "SE6-CTRL" ] || [ "$product" = "SE6 CTRL" ] \
		...
		if [ -f /etc/systemd/system/multi-user.target.wants/SophonHDMI.service ]; then
			...
			popd
			cp /root/se_ctrl/acpi/* /etc/acpi/
			systemctl restart acpid
		fi

		if [ ! -f /root/se_ctrl/script/installed ]; then
			# copy script at first
			cp /root/se_ctrl/script/reboot_all /usr/sbin/
			cp -r /root/se_ctrl/script /home/linaro/
			...
			sync
			touch /root/se_ctrl/script/installed
		fi
		...
		sectr_set_aiu_ipaddr ${lan1ip} ${lan2ip}
		...
		if [ "$coreip" = "0.0.0.0" ] || [ "$coreip" = "" ];then
			...
			# cores init only
			if [ ! -f /root/se_ctrl/script/installed ]; then
				cp -r /root/se_ctrl/tftp_update /home/linaro
				cp -r /root/se_ctrl/script /home/linaro
