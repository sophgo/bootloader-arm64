...
static int bitmain_spi_child_pre_probe(struct udevice *dev)
{
	struct spi_slave *slave = dev_get_parent_priv(dev);

	/* max read/write sizes */
	slave->max_read_size = 65536;
	slave->max_write_size = 65536;
			    ...
			    const void *dout, void *din, unsigned long flags)
{
	...
	debug("spi_xfer: base:0x%lx bus:%i cs:%i bitlen:%i bytes:%i flags:%lx freq:%i dout:0x%p din:0x%p\n",
	      priv->ctrl_base, bus->seq_, slave_plat->cs, bitlen, bytes, flags, priv->freq, dout, din);
...
U_BOOT_DRIVER(bitmain_spi) = {
	...
	.ops	= &bitmain_spi_ops,
	.priv_auto = sizeof(struct bitmain_spi_priv),
	.child_pre_probe = bitmain_spi_child_pre_probe,
