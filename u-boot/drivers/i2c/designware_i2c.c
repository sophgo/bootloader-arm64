...
static struct dw_scl_sda_cfg bm_config;
#endif
				 ...
				 unsigned int speed, unsigned int bus_clk)
{
	...
	int ret;
#ifdef CONFIG_ARCH_BITMAIN
	unsigned int clk_freq = priv->i2c_clk_frq ? (priv->i2c_clk_frq / 1000000) : IC_CLK;

	bm_config.hs_hcnt = (clk_freq * MIN_HS_SCL_HIGHTIME) / NANO_TO_MICRO;
	bm_config.hs_lcnt = (clk_freq * MIN_HS_SCL_LOWTIME) / NANO_TO_MICRO;
	bm_config.ss_hcnt = (clk_freq * MIN_SS_SCL_HIGHTIME) / NANO_TO_MICRO;
	bm_config.ss_lcnt = (clk_freq * MIN_SS_SCL_LOWTIME) / NANO_TO_MICRO;
	bm_config.fs_hcnt = (clk_freq * MIN_FS_SCL_HIGHTIME) / NANO_TO_MICRO;
	bm_config.fs_lcnt = (clk_freq * MIN_FS_SCL_LOWTIME) / NANO_TO_MICRO;
	priv->scl_sda_cfg = &bm_config;
#endif
...
#else
#ifdef CONFIG_ARCH_BITMAIN
	rate = i2c->i2c_clk_frq ? (i2c->i2c_clk_frq / 1000000) : IC_CLK;
#else
	rate = IC_CLK;
#endif
#endif
...
int designware_i2c_of_to_plat(struct udevice *bus)
{
	...
	priv->i2c_clk_frq = fdtdec_get_int(blob, node, "bus-clocks", 100000000);
#endif
...
#if CONFIG_IS_ENABLED(CLK) && !defined(CONFIG_ARCH_BITMAIN)
