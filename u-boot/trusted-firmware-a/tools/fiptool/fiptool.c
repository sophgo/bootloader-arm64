...
static int pack_images(const char *filename, uint64_t toc_flags, unsigned long align)
{
	...
	for (desc = image_desc_head; desc != NULL; desc = desc->next) {
		...
		xfwrite(image->buffer, image->toc_e.size, fp, filename);
#ifdef CONFIG_ARCH_BITMAIN
		/* Add CRC to the end of each BLx image */
		unsigned short crc;
		unsigned int pattern;

		if (fseek(fp, image->toc_e.offset_address + image->toc_e.size - 4, SEEK_SET)) {
			log_errx("Failed to set file position");
		} else {
			crc = crc16_ccitt(0, image->buffer, image->toc_e.size - 4);
			pattern = crc | (0xCAFE << 16);
			xfwrite(&pattern, 4, fp, filename);

			printf("%s: CRC=0x%X, offset=0x%lx\n", desc->name, pattern,
			       image->toc_e.offset_address + image->toc_e.size - 4);
		}
	}
	putchar('\n');
#else
