...
static int load_image(unsigned int image_id, image_info_t *image_data)
{
	...
	uint32_t i, cur_crc, signature = 0;
#endif
...
#ifdef CONFIG_ARCH_BITMAIN
	/* minus 4 bytes to exclude crc pattern */
	image_size -= 4;

	for (i = 0; i < 4; i++)
		signature |= ((*(uint8_t *)(image_base + image_size + i)) << (i * 8));

	if (((signature >> 16) & 0xFFFF) != 0xCAFE) {
		ERROR("Image id_%u: signature corruption (wrong-sig=0x%x)\n",
		      image_id, signature >> 16);
		io_result = -EBADF;
		goto exit;
	}

	cur_crc = crc16_ccitt(0, (unsigned char *)image_base, image_size);
	if (cur_crc != (signature & 0xFFFF)) {
		ERROR("Image id_%u: data corruption (wrong-crc=0x%x, expect=0x%x)\n",
		      image_id, (signature & 0xFFFF), cur_crc);
		io_result = -EBADF;
		goto exit;
	}
#endif
