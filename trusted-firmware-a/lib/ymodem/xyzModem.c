...
CYGACC_COMM_IF_GETC_TIMEOUT (char chan, char *c)
{
  ...
  ulong ts = timeout_init_us(xyzModem_CHAR_TIMEOUT * 1000);
  int rc = -1;

  while (rc == -1)
    {
      rc = getchar();
      if (timeout_elapsed(ts))
        break;
    }
  if (rc != -1)
    {
...
#if defined(XYZ_DEBUG)
/*
 * Note: this debug setup works by storing the strings in a fixed buffer
 *
 * WARNING: this debug buffer has no wrap round design, it will crash data
 * when receiving large file !!!
 */
...
zm_dprintf(char *fmt, ...)
{
	...
	va_start(args, fmt);
	len = vsnprintf(zm_out, sizeof(zm_debug_buf) - (zm_debug_buf - (char *)&zm_debug_buf), fmt, args);
	va_end(args);
	zm_out += len;
	return len;
}
