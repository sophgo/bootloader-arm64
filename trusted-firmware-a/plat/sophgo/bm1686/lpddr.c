...
static uint32_t ddr_phy_init(uint32_t cfg_base, uint32_t ddr_index)
{
	...
	if (ddr_index == DDR_CTRL0_A_INDEX) {
		for (i = 0; i < ARRAY_SIZE(pin_swap_0); i++)
			dwc_phy_apb_write16(cfg_base, pin_swap_0[i].offset, pin_swap_0[i].value);
	} else if (ddr_index == DDR_CTRL0_B_INDEX) {
		for (i = 0; i < ARRAY_SIZE(pin_swap_1); i++)
			dwc_phy_apb_write16(cfg_base, pin_swap_1[i].offset, pin_swap_1[i].value);
	} else if (ddr_index == DDR_CTRL1_INDEX) {
		for (i = 0; i < ARRAY_SIZE(pin_swap_2); i++)
			dwc_phy_apb_write16(cfg_base, pin_swap_2[i].offset, pin_swap_2[i].value);
	} else if (ddr_index == DDR_CTRL2_INDEX) {
		for (i = 0; i < ARRAY_SIZE(pin_swap_3); i++)
			dwc_phy_apb_write16(cfg_base, pin_swap_3[i].offset, pin_swap_3[i].value);
	}
	lpddr_print("phy pll\n");
...
start:
	...
	bm_sysdma_memcpy(NS_SRAM_UNUSED_BASE, NS_DRAM0_BASE, size, 0);
#else
	size = GROUP0_RANK(rank) == RANK1 ?
	...
	// init ddr0a ecc
	bm_sysdma_memcpy(NS_SRAM_UNUSED_BASE,
