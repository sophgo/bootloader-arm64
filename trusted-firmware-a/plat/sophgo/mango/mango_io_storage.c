...
static io_block_spec_t fip_in_spi_dmmr_block_spec = {
	.length = PLAT_MANGO_FIP_MAX_SIZE,
};
...
static int check_fip_source(const uintptr_t spec)
{
	...
	case FIP_SRC_SPIF:
		...
		fip_in_spi_dmmr_block_spec.offset = FLASH1_BASE + spif_offset;
...
void bm_locate_next_image(void)
{
	...
	if ((bm_gpio_read(BIT_MASK_GPIO_BOOT_SEL) & BOOTSEL_SKIP_SD) &&
	    bm_sd_card_detect() &&
	    (bm_sd_init(SD_USE_PIO) == 0)) {
		FRESULT f_ret;
		FATFS SDC_FS;
		FIL fp;
	...
	if (fip_src == FIP_SRC_SDFT && bm_sd_init(SD_USE_PIO))
