...
static const int cci550_map[] = {
	PLAT_MANGO_CCI_CLUSTER0_SL_IFACE_IX,
	PLAT_MANGO_CCI_CLUSTER1_SL_IFACE_IX
};

static struct mango_cci_slave {
	int cluster_id;
	int status;
} cci_slave[] = {
	{
		.cluster_id = PLAT_MANGO_CCI_CLUSTER0_SL_IFACE_IX,
		.status = MANGO_CORE_DOWN
	},
	{
		.cluster_id = PLAT_MANGO_CCI_CLUSTER1_SL_IFACE_IX,
		.status = MANGO_CORE_DOWN
	}
};
...
void mango_cci_init(void)
{
	...
	cci_init(CCI_CFG_BASE, cci550_map, ARRAY_SIZE(cci550_map));
}

/* set cci enable/disable for cluster1/2 */
void mango_cci_enable(int en)
{

	int master_id = MPIDR_AFFLVL1_VAL(read_mpidr());

	if (en && !cci_slave[master_id].status) {
		cci_slave[master_id].status = MANGO_CORE_UP;
		cci_enable_snoop_dvm_reqs(master_id);
	} else if (!en && cci_slave[master_id].status) {
		cci_slave[master_id].status = MANGO_CORE_DOWN;
		cci_disable_snoop_dvm_reqs(master_id);
	}
}
...
static int bm_pwr_domain_on(u_register_t mpidr)
{
	...
	uint64_t *hold_base = (uint64_t *)PLAT_MANGO_HOLD_BASE;

	print_entry(__func__);

	hold_base[pos] = PLAT_MANGO_HOLD_STATE_GO;
	dsb();
	sev();
...
void bm_pwr_domain_on_finish(const psci_power_state_t *target_state)
{
	...
	mango_cci_enable(MANGO_CORE_UP);
...
#ifdef MCU_I2C_DEV
	/* control mcu to power off mango */
...
{
	uintptr_t *mailbox = (void *)PLAT_MANGO_TRUSTED_MAILBOX_BASE;
