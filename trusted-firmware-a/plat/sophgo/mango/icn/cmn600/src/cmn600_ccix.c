    ...
    uint8_t ldid, uint32_t node_id, uint8_t local_remote)
{
	...
	/* Clear the correspoding register*/
	hnf_reg->RN_PHYS_ID[reg_index] &=
		(reg_bit_offset == 0) ? 0xFFFFFFFF00000000 : 0x00000000FFFFFFFF;
...
#if 1
	/* TODO: add this log, cmn will work, else ccix cannot work */
	FWK_LOG_INFO(MOD_NAME
		     "Exchanging protocol credits for link %d...", link_id);
	NOTICE("WORKAROUND: add some log, wait cmn ccix setup done\n");
	NOTICE("Exchanging protocol credits for link %d...\n", link_id);
#endif
