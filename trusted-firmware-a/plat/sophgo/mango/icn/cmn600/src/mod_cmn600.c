...
#define CHIP0_PCIE0_BUS_NUM  0x00
#define CHIP0_PCIE1_BUS_NUM  0x01

#define CHIP1_PCIE0_BUS_NUM  0x10
#define CHIP1_PCIE1_BUS_NUM  0x11

/* 3'b000: 128B; 3'b001: 256B; 3'b010: 512B */
#define CCIX_MAX_PACKET_SIZE 2
#define LINK_ID              0
#define CCIX_OPT_TLP         0
#define CCIX_MSG_PACK_ENABLE 1
#define CCIX_TC              1
...
/*
 * Configure XP that connects RNF
 * Port 1: snoop and DVM domain control register
 */
static void process_node_mxp(void)
{
	struct mango_topology *tp = mango_get_topology();
	int *rv_map = tp->socket[tp->self].rv_map;
	int i, xp_idx;
	struct cmn600_mxp_reg *xp;
	const struct mod_cmn600_config *config = ctx->config;

	for (i = 0; i < ARRAY_SIZE(tp->socket[tp->self].rv_map); ++i) {
		xp_idx = get_xp_index_from_cluster_id(i);

		xp = get_child_node(config->base, ctx->root, xp_idx);
		fwk_assert(get_node_type(xp) == NODE_TYPE_XP);

		xp->P_SYSCOREQ_CTL[1] = rv_map[i] ? 0x0f : 0x00;
	}
}
...
static void process_node_hnf(struct cmn600_hnf_reg *hnf)
{
    ...
    bit_pos = (CMN600_HNF_CACHE_GROUP_ENTRY_BITS_WIDTH / cal_mode_factor) *
        (logical_id % (CMN600_HNF_CACHE_GROUP_ENTRIES_PER_GROUP *
                       cal_mode_factor));
#endif
    for (int index = 0; index < config->hnf_count; index++) {
        if(node_id == config->hnf_table[index]) {
            group = index / CMN600_HNF_CACHE_GROUP_ENTRIES_PER_GROUP;
            bit_pos = (CMN600_HNF_CACHE_GROUP_ENTRY_BITS_WIDTH) *(index % (CMN600_HNF_CACHE_GROUP_ENTRIES_PER_GROUP));
            break;
        }
    }
    ...
    /* Disable DMT/DCT function in all HNFs */
    hnf->CFG_CTL |= CMN600_DMT_OFF;
    hnf->AUX_CTL |= CMN600_DCT_OFF;

    /* Configure the system cache RAM PPU */
    hnf->PPU_PWPR = CMN600_PPU_PWPR_POLICY_ON |
#ifdef CONFIG_CMN_DISABLE_SLC
                    CMN600_PPU_PWPR_OPMODE_SFONLY |
#else
                    CMN600_PPU_PWPR_OPMODE_FAM |
#endif
                    CMN600_PPU_PWPR_DYN_EN;

    hnf->QOS_RESERVATION = 31UL | (31UL << 8) | (31UL << 16) | (31UL << 24) |
	    (1UL << 32);
}
        ...
        for (node_idx = 0; node_idx < node_count; node_idx++) {
            ...
            if (is_child_external(xp, node_idx)) {
                ...
                case NODE_TYPE_HN_F:
                    ...
                    ctx->hnf_offset[ctx->hnf_count++] = (unsigned long)node;
                    break;
...
int cmn600_setup_sam(struct cmn600_rnsam_reg *rnsam)
{
	...
	uint64_t hncountvalue = 0;
    for (int i = 0; i < CMN600_MAX_NUM_SCG; i++){
	hncountvalue += ((uint64_t)(ctx->hnf_count / CMN600_MAX_NUM_SCG)) << 8 * i;
    }
	rnsam->SYS_CACHE_GRP_HN_COUNT = hncountvalue;
...
static int cmn600_setup(void)
{
    ...
    ctx->chip_id = mango_get_socket_id();
    ...
    struct mod_cmn600_ccix_remote_node_config *config)
{
    ...
    if(ctx->chip_id == 0) {
	    ...
	    ctx->cxg_ha_node_id = NODE_ID_CCIX0;
	    ctx->cxg_ra_reg = (void *)0x7070004000;
	    ctx->cxg_ha_reg = (void *)0x7070014000;
	    ctx->cxla_reg = (void *)0x7070024000;

	    config->smp_mode = true;
	    config->ccix_link_id = LINK_ID;
	    config->remote_rnf_count = 16;
	    config->pcie_bus_num = CHIP0_PCIE0_BUS_NUM;
	    config->ccix_max_packet_size = CCIX_MAX_PACKET_SIZE;
	    config->ccix_msg_pack_enable = true;
	    config->ccix_tc = CCIX_TC;
	    config->ccix_opt_tlp = false;
	    config->remote_ha_count = 1;
	    config->remote_sa_count = 0;
	    config->remote_ha_mmap_count = 1;
	    config->remote_ha_mmap[0].ha_id = 0x1;
	    config->remote_ha_mmap[0].base = (512ULL * FWK_GIB);
	    config->remote_ha_mmap[0].size = (512ULL * FWK_GIB);
    } else {
	    ctx->cxg_ha_id = ctx->chip_id;
	    ctx->cxg_ha_node_id = NODE_ID_CCIX1;
	    ctx->cxg_ra_reg = (void *)0xF071804000;
	    ctx->cxg_ha_reg = (void *)0xF071814000;
	    ctx->cxla_reg = (void *)0xF071824000;

	    config->smp_mode = true;
	    config->ccix_link_id = LINK_ID;
	    config->remote_rnf_count = 16;
	    config->pcie_bus_num = CHIP1_PCIE1_BUS_NUM;
	    config->ccix_max_packet_size = CCIX_MAX_PACKET_SIZE;
	    config->ccix_msg_pack_enable = true;
	    config->ccix_tc = CCIX_TC;
	    config->ccix_opt_tlp = false;
	    ...
	    config->remote_sa_count = 0;
	    config->remote_ha_mmap_count = 1;
	    config->remote_ha_mmap[0].ha_id = 0x0;
	    config->remote_ha_mmap[0].base = 0x0;
	    config->remote_ha_mmap[0].size = (512ULL * FWK_GIB);
    }
