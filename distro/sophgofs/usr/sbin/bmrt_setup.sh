...
function install_prepackages()
{
	...
	if [ ! -f /root/se6_ctrl/debs/installed ] ; then
		...
		product=$(cat /sys/bus/i2c/devices/1-0017/information | grep model | awk -F \" '{print $4}')
		if [ "$product" = "SE6-CTRL" ] && [ -d /root/se6_ctrl/debs ]; then
			/root/se6_ctrl/debs/tftpd_setup.sh
...
function se6_init()
{
	...
	if [ "$product" = "SE6-CTRL" ] ; then
		...
		if [ -f /etc/systemd/system/multi-user.target.wants/bmSysMonitor.service ]; then
			systemctl stop bmSysMonitor.service
			systemctl disable bmSysMonitor.service
		...
		wanip=$(ifconfig enp3s0 | grep "inet "|awk '{print $2}'|awk -F . '{printf("%d.%d\n", $1,$2)}' )
