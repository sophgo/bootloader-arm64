...
class serialCom():
        ...
        while self.rec_state:
            while self.serial.inWaiting()>0:
                ...
                if len(str(self.out).split(': ')) > 1:
                    self.simstatus = (str(self.out).split(': ')[1]).split('\\')[0]
    ...
    def monitor(self):
        ...
        while True:
            ...
            print (result)
            if "ppp0" in result:
                print('4g network is available...self.confg_changed is ' + str(self.confg_changed))
                    ...
                    if (len(data) > 2) :
                        ...
                        if cmdlist[0] == "get":
                            if cmdlist[1] == "ltefirst":
                                conn.send(bytes(self.ltefirst, encoding = "utf8"))
                            elif cmdlist[1] == "simstatus":
                                print('simstatus :' + self.simstatus)
                                if self.simstatus != 'READY':
                                    conn.send(bytes('SIM CARD FAILED', encoding = "utf8"))
                                else:
                                    conn.send(bytes('SIM CARD OK', encoding = "utf8"))

                        elif cmdlist[0] == "set":
                            if cmdlist[1] == "ltefirst":
                                if cmdlist[2] != self.ltefirst:
                                    self.confg_changed = True
                                    self.conf.set("base","ltefirst", cmdlist[2])
