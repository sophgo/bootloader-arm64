...
std::queue<decode_info_t *> g_image_vpp_queue[MAX_THREAD_NUM];
...
void *video_decoder_pthread(void *arg){
    ...
    while(!g_exit_flag){
        ...
        decode_info_t *dec_info = (decode_info_t *)malloc(sizeof(decode_info_t));
        ...
        codec_context = reader->getCodecContext();
        dec_info->frame = frame;
        dec_info->coded_width = codec_context->coded_width;
        dec_info->coded_height = codec_context->coded_height;
        g_vpp_queue_lock[index].lock();
        g_image_vpp_queue[index].push(dec_info);
...
void *video_process_pthread(void *arg){
    ...
    while(!g_exit_flag){
        ...
        g_vpp_queue_lock[index].lock();
        dec_info = g_image_vpp_queue[index].front();
        ...
        if(encode_pixel_format == AV_PIX_FMT_YUV420P){
            ...
            AVFrameConvert(g_bmHandle, dec_info->frame, out_frame, dec_info->coded_width, dec_info->coded_height ,height, width,
                            encode_pixel_format, g_enable_mosaic, g_enable_watermark, g_watermark);
            if(!(&out_frame)){
        ...
        if(encode_pixel_format == AV_PIX_FMT_YUV420P){
            av_frame_unref(dec_info->frame);
            av_frame_free(&dec_info->frame);
        }
        ...
        if(encode_pixel_format == AV_PIX_FMT_YUV420P){
            ...
            g_image_enc_queue[index].push(dec_info->frame);
        free(dec_info);
