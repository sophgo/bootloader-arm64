...
function create_spif_script()
{
	...
	if [ ! -e ${SOURCE_FILES_PATH}/spi_flash.bin ]; then
		...
		script_update "echo update section A done"
		script_update "if test \"\$chip_type\" = \"bm1684\"; then sf update ${IN_ADDR_B} 0x200000 0x200000; else sf update ${IN_ADDR} 0x200000 0x200000; fi"
		script_update "echo update section B done"
		script_update "else"
